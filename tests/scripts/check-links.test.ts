...
describe('GitHub heading slugs', () => {
  test('lower-cases, drops punctuation, keeps double hyphens from removed dashes', () => {
    expect(slugify('Combat opening — confirmed for v0.01')).toBe('combat-opening--confirmed-for-v001');
    expect(slugify('9. Verification and acceptance')).toBe('9-verification-and-acceptance');
    expect(slugify('Rules-review workflow accepted for trial')).toBe('rules-review-workflow-accepted-for-trial');
    expect(slugify('Use `pnpm check` **now**')).toBe('use-pnpm-check-now');
    expect(slugify('Initiative groups (confirmed app model)')).toBe('initiative-groups-confirmed-app-model');
  });
...
describe('link extraction', () => {
  test('finds inline links and reference definitions, skips code and footnotes', () => {
    const links = extractLinks('See [a](x.md#h) and `[no](code.md)`\n![img](pic.png)\n[ref]: other.md\n[^1]: footnote [b](y.md)\n```\n[c](z.md)\n```\n');
...
describe('check-links on a fixture tree', () => {
  ...
  test('reports only the broken relative links with file, line and reason', async () => {
    ...
    expect(broken).toEqual([
      { file: 'docs/a.md', line: 5, target: '#missing', reason: 'no heading #missing in docs/a.md' },
      { file: 'docs/a.md', line: 5, target: 'c.md', reason: 'missing file docs/c.md' },
      { file: 'docs/a.md', line: 5, target: '../../etc/passwd', reason: 'points outside the repository' },
    ]);
  });
});
