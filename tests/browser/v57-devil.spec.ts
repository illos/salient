...
test('V57 new Devil choices persist and Wings remains readable with its conditional limits', async ({ page }) => {
  const stamp = crypto.randomUUID().slice(0, 8);
  const email = `v57-${stamp}@example.test`;
  await register(page, `Devil ${stamp}`, email);
  await page.goto('/characters/new/wizard');
  const ancestry = () => page.getByRole('navigation', { name: 'Steps' })
    .getByRole('button', { name: /Ancestry/ }).click();
  await ancestry();
  ...
  await page.getByRole('navigation', { name: 'Steps' })
    .getByRole('button', { name: /Determine Details/ }).click();
  ...
  const readback = await promisify(execFile)('pnpm', [
  ...
  expect(saved.selections).toContainEqual(expect.objectContaining({
    decisionId: 'ancestry.devil.silver-tongue-skill', value: 'Read Person',
  }));
  expect(saved.selections).toContainEqual(expect.objectContaining({
    decisionId: 'ancestry.devil.purchased-traits', value: ['Wings', 'Barbed Tail'],
  }));
