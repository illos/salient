...
/**
 * The build-time equipment surface: the kit, its equipment text, and the Wealth and Renown values
 * on the derived baseline. The sheet prints exactly these. This is **not** a mutable inventory —
 * the application has none — so comparing it across advancement and restore proves that baseline
 * equipment is conserved, and nothing about items a player might add or spend in play.
 */
...
test('V46: changing a purchased trait or the ancestry removes its grants across a reload', async ({
  ...
  try {
    ...
    // Captured from the reloaded sheet, so the retained readback reports what was read back rather
    // than repeating the constants this step already asserted.
    const flyingRead = {
      movementModes: (sheet.build!.baseline!.movementModes ?? []).map(mode => mode.mode),
      conditionalEffects: effects(sheet.build!.baseline!),
      ancestryAbilities: sheet.abilities
        .filter(ability => ability.kind === 'ancestry')
        .map(ability => ability.name),
    };
...
test('V46: a flying devil advances to Berserker Fury two with live state and history intact', async ({
  ...
  try {
    ...
    // Anchor the comparison first. `carried()` returns nulls for a kitless build, so equality
    // alone would be satisfied by the kit vanishing at both levels; this pins what is conserved.
    expect(carried(before.build!.baseline!)).toMatchObject({ kit: 'Mountain' });
    expect(carried(before.build!.baseline!).equipment, 'kit equipment text').toBeTruthy();
      ...
      JSON.stringify(
        {
          ...
          note:
            '"carried" is build-time equipment only: the kit, its equipment text, and the Wealth' +
            ' and Renown values on the derived baseline. A mutable inventory is not implemented —' +
            ' no table in the composed convex/schema.ts holds items — so this records that the' +
            ' baseline equipment is conserved, not that any in-play possession was preserved.',
        },
        null,
        1,
      ),
