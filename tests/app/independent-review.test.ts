...
test('independent: persisted session deletion revokes already issued identity across all private surfaces', async () => {
  ...
  const campaignId = await owner.client.mutation(api.campaigns.create, {
    name: 'Campaign',
    commandId: 'review-campaign',
  });
  const characterId = await owner.client.mutation(api.characters.create, {
    commandId: 'review-character',
    authored: { name: 'Hero', appearance: '', biography: '', notes: 'secret' },
  });
  await t.mutation(components.betterAuth.adapter.deleteOne, {
    input: { model: 'session', where: [{ field: '_id', value: owner.session._id }] },
  });
  ...
  await expect(
    owner.client.mutation(api.sessions.start, {
      campaignId,
      selectedPlayerIds: [],
      commandId: 'review-revoked-start',
    }),
...
test('independent: stolen request identity cannot withdraw or review another applicant', async () => {
  ...
  await outsider.client.mutation(api.campaigns.requestJoin, {
    shareCode: shareCode!,
    commandId: 'review-request',
  });
  const requestId = (await outsider.client.query(api.campaigns.myRequests, {}))[0].id;
  await expect(
    owner.client.mutation(api.campaigns.withdrawRequest, {
      requestId,
      commandId: 'review-steal-withdraw',
    }),
  ).rejects.toThrow('unavailable');
  await expect(
    outsider.client.mutation(api.campaigns.declineRequest, {
      requestId,
      commandId: 'review-steal-decline',
    }),
  ).rejects.toThrow('unavailable');
  expect(
    (await owner.client.query(api.campaigns.get, { campaignId })).pendingRequests,
  ).toHaveLength(1);
  await expect(
    t.query(api.characters.get, {
      characterId: await owner.client.mutation(api.characters.create, {
        commandId: 'review-char',
        authored: { name: 'Hero', appearance: '', biography: '', notes: 'secret' },
      }),
    }),
  ).rejects.toThrow('Sign in');
...
test('independent: simultaneous same-command submissions persist exactly once', async () => {
  ...
  const ids = await Promise.all([
    owner.client.mutation(api.campaigns.create, args),
    owner.client.mutation(api.campaigns.create, args),
  ]);
  ...
  const starts = await Promise.allSettled([
    owner.client.mutation(api.sessions.start, {
      campaignId,
      selectedPlayerIds: [],
      commandId: 'review-start-one',
    }),
    owner.client.mutation(api.sessions.start, {
      campaignId,
      selectedPlayerIds: [],
      commandId: 'review-start-two',
    }),
  ]);
...
test('independent: cyclic visibility intent after a lost reply applies the new command', async () => {
  ...
  async function attempt(visible: boolean, lostReply: boolean) {
    ...
    await owner.client.mutation(api.foes.setVisible, {
      campaignId,
      foeId,
      visible,
      commandId: ids.forPayload(key),
    });
