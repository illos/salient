...
function build(
  ...
  return {
    ...
    'ancestry.revenant.former-life': former,
    [revenantPurchaseId(former)]: traits,
    ...extra,
  };
}
...
test('V79 Former Life gives only size; small Revenants have the additional ancestry point', () => {
  for (const [former, size, traits] of [
    ['Hakaan', '1L', ['Bloodless']],
    ['Polder', '1S', ['Bloodless', 'Undead Influence']],
    ['Time Raider', '1M', ['Bloodless']],
  ] as const) {
    ...
    assert.deepEqual(
      hero.damageWeaknesses?.map(i => [i.damageType, i.value.value]),
      [['fire', 5]],
    );
...
test('V79 multiple different Previous Life purchases share the Revenant budget without doubling immunity', () => {
  const hero = evaluate(
    build('Polder', ['Corruption Immunity', 'Graceful Retreat', 'Reactive Tumble']),
  ).baseline!;
...
test('V79 borrowed Psionic Gift requires and grants exactly one nested ability', () => {
  ...
  const hero = evaluate(
    build('Time Raider', ['Psionic Gift'], {
  ...
  const artisan = evaluate(
    build('Orc', ['Passionate Artisan', 'Grounded'], {
      'ancestry.revenant.orc.passionate-artisan.skills': ['Alchemy', 'Architecture'],
    }),
  );
...
test('V79 Vengeance Mark grants placement, removal and Detonate Sigil and revokes them on replacement', () => {
  ...
  for (const name of ['Vengeance Mark', 'Vengeance Mark: Remove Sigil', 'Detonate Sigil'])
    assert.equal(hero.abilities.filter(a => a.name === name).length, 1);
  assert.ok(
    !evaluate(build('Human', ['Bloodless'])).baseline!.abilities.some(
      a => a.name === 'Detonate Sigil',
    ),
  );
});
...
test('V79 paid former traits contribute permanent values for kit and no-kit builds', () => {
  const examples: [string, string[], string, number][] = [
    ['Dwarf', ['Spark Off Your Skin'], 'staminaMaximum', 36],
    ['Human', ['Staying Power'], 'recoveriesMaximum', 12],
    ['Devil', ['Beast Legs'], 'speed', 6],
    ['Wode Elf', ['Otherworldly Grace'], 'savingThrowThreshold', 5],
    ['Memonek', ['Lightning Nimbleness'], 'speed', 7],
  ];
  for (const [former, traits, field, value] of examples) {
    const hero = evaluate(build(former, traits)).baseline!;
    assert.equal(
      (hero as unknown as Record<string, { value: number }>)[field].value,
      value,
      former,
    );
  }
...
test('V79 former ancestry replacement prunes borrowed descendants and Wyrmplate cannot be invented', () => {
  const initial = build('Time Raider', ['Psionic Gift'], {
    'ancestry.revenant.time-raider.psionic-gift.ability': 'Psionic Bolt',
  });
  const changed = pruneUnavailable(
    {
      ...initial,
      'ancestry.revenant.former-life': 'Dwarf',
      [revenantPurchaseId('Dwarf')]: ['Grounded', 'Stone Singer'],
    },
    definitions,
  );
  ...
  assert.notEqual(
    evaluate(
      build('Dragon Knight', ['Prismatic Scales'], {
        'ancestry.revenant.dragon-knight.prismatic-scales-immunity': 'acid',
      }),
    ).status,
    'complete',
  );
