...
test('source-grounded fixtures and ordinary monster/hero grammar', async () => {
  ...
  assert.deepEqual(slam.tiers[2], [{ kind: 'damage', amount: { constant: 9, characteristic: 'M' } }, { kind: 'push', distance: 4 }]);
  ...
  assert.deepEqual(bury.tiers[0][1], { kind: 'condition', characteristic: 'M', threshold: 0, condition: 'bleeding', duration: 'save ends' });
...
test('homebrew renaming, numeric changes and composition use the same grammar', async () => {
  ...
  const homebrew: AbilitySource = {
    ...original, id: 'homebrew:ember-hook', name: 'Ember Hook', cost: '3 Ferocity',
    text: original.text.replace('Bury the Point', 'Ember Hook').replace('2 [Malice](scc.v1:mcdm.monsters.v1/rule.monster/malice)', '3 Ferocity')
      .replace('5 damage;', '10 fire damage; push 2;'),
    tiers: ['10 fire damage; push 2; M < 0 bleeding (save ends)', original.tiers![1], original.tiers![2]],
  };
  const result = parseAbility(homebrew);
  assert.deepEqual(result.diagnostics, []);
  assert.deepEqual(result.cost, { resource: 'ferocity', amount: 3 });
  assert.deepEqual(result.tiers[0].slice(0, 2), [{ kind: 'damage', amount: { constant: 10 }, damageType: 'fire' }, { kind: 'push', distance: 2 }]);
});
...
test('contradictory cost and extra title mechanics cannot be skipped', async () => {
  ...
  assert.ok(parseAbility({ ...s.abilities['warrior:bury-the-point'], cost: '1 Malice' }).diagnostics.some(value => value.includes('title')));
  assert.ok(parseAbility({ ...s.abilities['fury:out-of-the-way'], cost: '1 Ferocity' }).diagnostics.some(value => value.includes('cost differs')));
  const original = s.abilities['warrior:spear-charge'];
  assert.ok(parseAbility({ ...original, text: original.text.replace('Signature Ability', 'Signature Ability; cannot target heroes') }).diagnostics.some(value => value.includes('title')));
});

test('hero selected options preserve additional effects and report manual requirements', async () => {
  const s = await loadScenario();
  for (const id of ['fury:out-of-the-way', 'fury:thunder-roar', 'fury:lines-of-force', 'mountain:pain-for-pain']) {
...
test('unseen monster ability retains an unsupported charge rider', async () => {
  ...
  const { entity, abilities } = await loadMonster('monster/human/statblock/human-raider', 'holdout', 'holdout');
...
test('full hero text preserves known JSON projection omissions', async () => {
  const magma = await loadHeroAbility('feature/ability/elementalist/level-6/magma-titan', 'holdout:magma');
  const stasis = await loadHeroAbility('feature/ability/talent/level-6/stasis-field', 'holdout:stasis');
  assert.ok(parseAbility(magma).diagnostics.some(value => value.includes('They have fire immunity 10.')));
  assert.ok(parseAbility(stasis).diagnostics.some(value => value.includes('targets each enemy in the area.')));
});
