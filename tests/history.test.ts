...
import { abandonRequest, createRun, currentState, navigate, readRun, submitCommand, withRunFile, writeRun } from '../src/history.ts';
import type { ManualCommand, Resolution, Scenario } from '../src/contracts.ts';

const scenario: Scenario = { name: 'history-only fixture', sourceRevision: 'test', notes: [], abilities: {}, state: { entities: {}, squads: {}, pending: [], round: 1, malice: 0 } };
const command: ManualCommand = { kind: 'manual', id: 'one', reason: 'Test history storage independently of rules', changes: [{ kind: 'malice', value: 3 }] };
  ...
  try {
    ...
    await withRunFile(path, async () => {
      ...
      const evaluate = async (state: Scenario['state']): Promise<Resolution> => {
        ...
        state.malice = 3;
        return { status: 'resolved', state, effects: [{ kind: 'malice', before: 0, after: 3, text: 'Recorded manual change' }], messages: [], unresolved: [] };
      };
      ...
      await assert.rejects(submitCommand(run, { ...command, reason: 'Different' }, evaluate), /conflict/);
...
test('failed modifier and interrupted request are explicit and never rerun on reopen', async () => {
  ...
  try {
    const run = createRun(scenario);
    const failed = await submitCommand(run, command, () => { throw new Error('Example failure'); });
    assert.equal(failed.status, 'failed'); assert.match(failed.error!, /Example failure/);
    assert.equal(currentState(run).malice, 0);
    const pending = createRun(scenario);
    pending.entries.push({ command, status: 'requested', requestedAt: new Date().toISOString(), before: scenario.state, after: scenario.state }); pending.cursor = 1;
    await writeRun(path, pending);
    const reopened = await readRun(path);
    let calls = 0;
    const unreachable = (): Resolution => { calls++; throw new Error('Must not execute'); };
    assert.equal((await submitCommand(reopened, command, unreachable)).status, 'requested');
    await assert.rejects(submitCommand(reopened, { ...command, id: 'two' }, unreachable), /unfinished/);
...
test('reject corrupt history and concurrent writers; failed pre-save never invokes modifier', async () => {
  ...
  try {
    const run = createRun(scenario);
    let calls = 0;
    await assert.rejects(submitCommand(run, command, () => { calls++; throw new Error('Never called'); }, { persist: async () => { throw new Error('Disk full'); } }), /Disk full/);
    assert.equal(calls, 0); assert.equal(run.cursor, 0); assert.equal(run.entries.length, 0);
    await withRunFile(path, async () => { await assert.rejects(withRunFile(path, async () => {}), /locked/); });
    await writeRun(path, run);
