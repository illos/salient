...
function assertUntouched(before: GameState, returned: GameState) {
...
test('real Spear Charge changes actual Fury Stamina and gains Ferocity once per round', () => {
  ...
  const second = resolveAbility(first.state, parsed('warrior:spear-charge'), {
    ...action(),
    id: 'again',
  });
  assert.equal(second.state.entities.fury.stamina, 22);
  assert.equal(second.state.entities.fury.resources.ferocity, 1);
  const newRound = applyManual(second.state, {
    kind: 'manual',
    id: 'round2',
    reason: 'Table begins round two',
    changes: [{ kind: 'round', value: 2 }],
  });
  const third = resolveAbility(newRound.state, parsed('warrior:spear-charge'), {
    ...action(),
    id: 'third',
  });
...
test('first becoming winded requests its d3 before committing any damage or resource gain', () => {
  const before = initial();
  before.entities.fury.stamina = 18;
  const missing = resolveAbility(before, parsed('warrior:spear-charge'), action());
  assert.equal(missing.status, 'needs-input');
  assertUntouched(before, missing.state);
  const done = resolveAbility(before, parsed('warrior:spear-charge'), {
    ...action(),
    ferocityRoll: 2,
  });
  ...
  const again = resolveAbility(done.state, parsed('warrior:spear-charge'), {
    ...action(),
    id: 'again',
  });
...
test('temporary Stamina absorbs damage but still triggers first-damage Ferocity', () => {
  const before = initial();
  before.entities.fury.temporaryStamina = 10;
...
test('Brutal Slam applies sourced tier damage and size bonus, preserving unresolved movement', () => {
  ...
  const complete = applyManual(done.state, {
    kind: 'manual',
    id: 'move',
    reason:
      'Table pushed warrior three squares; no collision, hazards, falling, or other triggers.',
    changes: [],
    completePendingIds: [done.state.pending[0].id],
  });
  assert.equal(complete.status, 'resolved');
  assert.equal(complete.state.pending.length, 0);
  assert.equal(
    complete.state.entities.warrior.stamina,
    7,
    'Manual movement completion never repeats damage',
  );
  const duplicate = applyManual(complete.state, {
    kind: 'manual',
    id: 'move-again',
  ...
  assertUntouched(complete.state, duplicate.state);
});
...
test('Mountain tier3 bonus applies to Melee Weapon rolled damage; movement may be declined', () => {
  ...
  assert.equal(
    resolveAbility(initial(), nonStrike, command).state.entities.warrior.stamina,
    0,
    'Kit damage does not require Strike keyword',
  );
  ...
  assert.equal(
    resolveAbility(initial(), alreadyIncluded, command).state.entities.warrior.stamina,
    4,
    'Already-included kit bonuses are not applied twice',
  );
});
...
test('Bury the Point spends shared Malice once and uses strict potency comparison', () => {
  const before = initial();
  before.malice = 2;
  const command = action('warrior', 'warrior:bury-the-point');
  command.roll = [8, 8];
  ...
  const weaker = structuredClone(before);
  weaker.entities.fury.characteristics.M = 1;
  ...
  assert.equal(noFunds.status, 'rejected');
  assertUntouched(initial(), noFunds.state);
});

test('minion damage changes squad pool and chooses actual casualties, never individual counters', () => {
  const before = initial(),
    command = slam();
  command.targetIds = ['spine-1'];
  command.roll = [8, 8];
  ...
  assert.equal(
    done.state.entities['spine-1'].stamina,
    5,
    'Member field is definition/display data, not individual health',
  );
  assert.deepEqual(
    ['spine-1', 'spine-2', 'spine-3'].map(id => done.state.entities[id].defeated),
    [true, true, true],
  );
...
test('three Spinecleavers use one tier result plus two free-strike bonuses and one push', () => {
  const command = action('spine-1', 'spine:axe');
  command.roll = [8, 8];
  command.facts.squadAttackers = ['spine-1', 'spine-2', 'spine-3'];
  command.facts.movement = {
    fury: { distance: 2, stabilityReduction: 2, collision: false, effectsConfirmed: true },
  };
  ...
  assertUntouched(initial(), excessive.state);
});
...
test('rejects malformed inputs, ownership, range, and invalid movement without partial application', () => {
  ...
  for (const command of invalidCommands) {
    const before = initial(),
      done = resolveAbility(before, parsed('warrior:spear-charge'), command);
    assert.equal(done.status, 'rejected');
    assertUntouched(before, done.state);
  }
  const command = slam();
  command.facts.movement = { warrior: { distance: 9, stabilityReduction: 0, collision: false } };
  const before = initial(),
    invalidPush = resolveAbility(before, parsed(command.abilityId), command);
  assert.equal(invalidPush.status, 'rejected');
  assertUntouched(before, invalidPush.state);
});

test('unsupported conditions, critical hits, traits, and Growing Ferocity are visible whole-action manual work', () => {
  const before = initial(),
    cases: [GameState, AbilityCommand][] = [];
  const crit = action();
  crit.roll = [10, 9];
  cases.push([initial(), crit]);
  const highFerocity = initial();
  highFerocity.entities.fury.resources.ferocity = 4;
  const push = slam();
  push.facts.furyFerocityPeak = 4;
  cases.push([highFerocity, push]);
  const bleeding = initial();
  bleeding.entities.warrior.conditions.push({
    name: 'bleeding',
    sourceId: 'earlier',
    duration: 'save ends',
  });
  cases.push([bleeding, action()]);
  const unknown = initial();
  unknown.entities.warrior.traits.push('source-trait:reflect-all-damage');
  cases.push([unknown, slam()]);
  for (const [state, command] of cases) {
    const done = resolveAbility(state, parsed(command.abilityId), command);
    assert.equal(done.status, 'manual-required');
    assert.equal(done.state.pending.length, 1);
    assert.deepEqual(done.state.entities, state.entities);
    assert.equal(done.state.malice, state.malice);
  }
...
test('manual changes and pending completion are atomic, including pooled health consistency', () => {
  const before = initial();
  const invalid = applyManual(before, {
    kind: 'manual',
    id: 'bad',
    reason: 'Batch must fail together',
    changes: [
      { kind: 'stamina', entityId: 'fury', value: 20 },
      { kind: 'malice', value: -1 },
    ],
  });
  assert.equal(invalid.status, 'rejected');
  assertUntouched(before, invalid.state);
  const individual = applyManual(before, {
    kind: 'manual',
    id: 'bad-minion',
    reason: 'Wrong health model',
    changes: [{ kind: 'stamina', entityId: 'spine-1', value: 0 }],
  });
  assert.equal(individual.status, 'rejected');
  const squad = applyManual(before, {
    kind: 'manual',
    id: 'squad',
    reason: 'Table resolves unsupported area damage',
    changes: [
      {
        kind: 'squad-stamina',
        squadId: 'spine-squad',
        value: 10,
        defeatedIds: ['spine-2', 'spine-3'],
      },
    ],
  });
  assert.equal(squad.status, 'resolved');
  assert.equal(squad.state.squads['spine-squad'].stamina, 10);
...
test('manual Fury damage includes explicit trigger history and does not double-grant Ferocity', () => {
  const before = initial();
  const missingBookkeeping = applyManual(before, {
    kind: 'manual',
    id: 'manual-hit',
    reason: 'First damage this round',
    changes: [
      { kind: 'stamina', entityId: 'fury', value: 26 },
      { kind: 'resource', entityId: 'fury', resource: 'ferocity', value: 1 },
    ],
  });
  assert.equal(missingBookkeeping.status, 'rejected');
  assertUntouched(before, missingBookkeeping.state);
  const recorded = applyManual(before, {
    kind: 'manual',
    id: 'manual-hit',
    reason: 'First damage this round',
    changes: [
  ...
  const malformed = {
    kind: 'manual',
    id: 'bad',
    reason: 'Missing entityId',
    changes: [{ kind: 'stamina', value: 20 }],
  };
...
test('all area targets take damage before ordered movement effects; other action types remain manual', () => {
  ...
  area.source.keywords.push('Area');
  area.source.target = 'Each enemy in the area';
  area.source.distance = '2 burst';
  const command = slam();
  command.targetIds = ['warrior', 'other'];
  command.facts.movement = {
    warrior: { distance: 1, stabilityReduction: 0, collision: false, effectsConfirmed: true },
    other: { distance: 1, stabilityReduction: 0, collision: false, effectsConfirmed: true },
  };
  const done = resolveAbility(before, area, command);
  assert.equal(done.status, 'resolved');
  assert.deepEqual(
    done.effects.map(e => [e.kind, e.targetId]),
    [
      ['damage', 'warrior'],
      ['damage', 'other'],
      ['push', 'warrior'],
  ...
  assert.equal(triggered.status, 'manual-required');
  assert.deepEqual(triggered.state.entities, before.entities);
});
