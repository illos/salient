...
const evaluate = (scenario: Scenario) => (state: GameState, command: Command) =>
  command.kind === 'manual'
...
test('unsupported real ability can be completed manually, saved, rewound, and deduplicated', async () => {
  ...
  let calls = 0;
  const modifier = (state: GameState, command: Command) => {
    calls++;
    return evaluate(scenario)(state, command);
  };
  ...
  try {
    await submitCommand(
      run,
      {
        kind: 'manual',
        id: 'turn-start',
        reason: 'Supplied first-turn d3 was 3',
        changes: [{ kind: 'resource', entityId: 'fury', resource: 'ferocity', value: 3 }],
      },
      modifier,
      { persist },
    );
    ...
    assert.equal(
      pending.after.entities.fury.resources.ferocity,
      3,
      'Unsupported action has not spent its cost',
    );
    assert.equal(
      pending.after.entities.warrior.stamina,
      15,
      'Recognized damage fragment is not prematurely applied',
    );
    ...
    const completion: Command = {
      ...
      id: 'finish-slide',
      reason:
        'Out of the Way tier2: 7 damage, spend 3 Ferocity; choose zero slide, no following movement.',
      changes: [
        { kind: 'resource', entityId: 'fury', resource: 'ferocity', value: 0 },
        { kind: 'stamina', entityId: 'warrior', value: 8 },
      ],
...
test('shared history records squad pool casualties and a surviving minion action', async () => {
  ...
  assert.deepEqual(
    ['spine-1', 'spine-2', 'spine-3'].map(id => hit.after.entities[id].defeated),
    [true, true, true],
  );
...
test('held-out monster ability and renamed numeric homebrew use the same grammar; extra hero effect stays visible', async () => {
  ...
  assert.deepEqual(
    parsed.tiers,
    [1, 2, 3].map(constant => [{ kind: 'damage', amount: { constant } }]),
  );

  const scenario = await loadScenario();
  const spear = scenario.abilities['warrior:spear-charge'];
  const homebrew: AbilitySource = {
    ...structuredClone(spear),
    id: 'homebrew:glass-pike',
    name: 'Glass Pike',
    ...
    tiers: spear.tiers!.map(tier => tier.replaceAll('4 damage', '7 damage')) as [
      string,
      string,
      string,
    ],
  ...
  assert.equal(
    hit.state.entities.fury.stamina,
    23,
    'Changed text produces changed damage without a named handler',
  );

  const thunder = await loadHeroAbility(
    'feature/ability/conduit/level-1/call-the-thunder-down',
    'holdout:thunder',
  );
  const unsupported = parseAbility(thunder);
  assert.ok(
    unsupported.diagnostics.some(d => /willing ally/.test(d)),
    'Full extra effect cannot disappear behind recognized tiers',
  );
