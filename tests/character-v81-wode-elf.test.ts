...
function build(traits:string[],fixture:Record<string,SelectionValue>=fury.selections) {
 ...
 return {...selections,'ancestry.choice':'Wode Elf','ancestry.wode-elf.purchased-traits':traits};
}
...
test('V81 The Wode Defends is granted only by its retained purchased trait',()=>{
 ...
 assert.ok(!evaluate(build(['Forest Walk','Quick and Brutal','Revisit Memory'])).baseline!.abilities.some(a=>a.name==='The Wode Defends'));
});
...
test('V81 manual traits remain distinct purchases and illegal budgets do not grant abilities',()=>{
 ...
 assert.deepEqual(hero.traits.map(t=>t.name).sort(),['Forest Walk','Quick and Brutal','Revisit Memory','Wode Elf Glamor']);
...
test('V81 replacing ancestry prunes the old list and its actions and save threshold',()=>{
 const changed=pruneUnavailable({...build(['The Wode Defends','Swift']),'ancestry.choice':'Human','ancestry.human.purchased-traits':['Staying Power','Perseverance']},definitions);
