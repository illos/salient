...
export async function requireMember(ctx: ReadCtx, campaignId: Id<"campaigns">, userId: Id<"users">) {
  const campaign = await ctx.db.get(campaignId);
  const membership = await ctx.db.query("memberships").withIndex("by_campaign_user", q => q.eq("campaignId", campaignId).eq("userId", userId)).unique();
...
export async function requireDirector(ctx: ReadCtx, campaignId: Id<"campaigns">, userId: Id<"users">) {
  return requireOwner(ctx, campaignId, userId);
}
export async function checkMembershipCapacity(ctx: ReadCtx, campaignId: Id<"campaigns">, userId: Id<"users">) {
  const members = await ctx.db.query("memberships").withIndex("by_campaign_user", q => q.eq("campaignId", campaignId)).take(100);
  const campaigns = await ctx.db.query("memberships").withIndex("by_user", q => q.eq("userId", userId)).take(50);
  if (members.length >= 100 || campaigns.length >= 50) throw new ConvexError("Prototype membership limit reached (100 members / 50 campaigns).");
}
