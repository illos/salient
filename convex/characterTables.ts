...
export const authoredValidator = v.object({ name: v.string(), appearance: v.string(), biography: v.string(), notes: v.string() });
export const selectionValidator = v.object({
  decisionId: v.string(), ownerBranchId: v.string(),
...
export const characterTables = {
  characters: defineTable({
    ownerId: v.id("users"), authored: authoredValidator, revision: v.number(),
    ...
    campaignId: v.union(v.id("campaigns"), v.null()), combatLocked: v.boolean(),
  }).index("by_owner", ["ownerId"]),
  characterRevisions: defineTable({
    characterId: v.id("characters"), revision: v.number(),
