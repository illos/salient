...
const overrides = ['CONVEX_DEPLOY_KEY', 'CONVEX_DEPLOYMENT_TOKEN', 'CONVEX_SELF_HOSTED_URL', 'CONVEX_SELF_HOSTED_ADMIN_KEY'];
if (!/^(anonymous|local):/.test(deployment) || !['127.0.0.1', 'localhost'].includes(url.hostname)
    || overrides.some(name => process.env[name] || config[name])
    || (process.env.CONVEX_DEPLOYMENT && process.env.CONVEX_DEPLOYMENT !== deployment)) {
  throw new Error('setup:local requires a configured local development deployment and no deployment-key overrides. Run convex init first; configure cloud environments separately.');
}
...
function convex(args: string[], input?: string) {
  ...
  return spawnSync(process.execPath, ['node_modules/convex/bin/main.js', ...args, '--env-file', '.env.local'], { input, encoding: 'utf8' });
}
function set(name: string, value: string) {
  const result = convex(['env', 'set', name], value);
  if (result.status !== 0) throw new Error(`Could not configure ${name}. Start pnpm dev:backend first. ${result.stderr}`);
  console.log(`${name} configured.`);
}
const existing = convex(['env', 'get', 'BETTER_AUTH_SECRET']);
if (existing.status !== 0) throw new Error('Could not read the existing auth secret. No settings changed; start the local backend and retry.');
...
if (/^VITE_LOCAL_PROXY=/m.test(env)) env = env.replace(/^VITE_LOCAL_PROXY=.*$/m, 'VITE_LOCAL_PROXY=true');
