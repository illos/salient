...
/** V133 Null levels two and three for every tradition, through authenticated public operations. */
...
import type levelOneLedger from '../../tests/fixtures/v103-null-expected.json';
import type sourceLedger from '../../tests/fixtures/v133-null-three-expected.json';
const levelOne = JSON.parse(
  readFileSync('tests/fixtures/v103-null-expected.json', 'utf8'),
) as typeof levelOneLedger;
const ledger = JSON.parse(
  readFileSync('tests/fixtures/v133-null-three-expected.json', 'utf8'),
...
/** Printed Self targets record or roll against the Null, never a separate target. */
const selfTargets = [
  'Blur',
  'Heat Sink',
  'Kinetic Shield',
...
const rolledRemainder: Record<string, RegExp> = {
  ...
  'Gravitic Strike': /./,
  'Entropic Field': /./,
  'Kinetic Shield': /temporary Stamina/i,
};
const compiled = new Set(['Force Redirected', 'Gravitic Strike', 'Entropic Field']);
type CompiledRead = {
  compiled: {
...
export async function runNullLevelThree({
  actors: { director, peer },
  run,
  runId,
}: ScenarioContext) {
  await run('Null levels 2–3: six builds, level edits and sixteen new uses persist', async () => {
    ...
    for (const [index, b] of builds.entries()) {
      const selections = {
        ...
        'class.null.level-2.perk': b.perk,
        [`class.null.level-2.${b.w.tradition.toLowerCase()}-ability`]: b.second,
        'class.null.level-3.ability-7': b.seventh,
      };
      ...
      for (const [ability, amount] of [
        ...
        assert.deepEqual(granted.cost, { resource: 'discipline', amount }, `${name} ${ability}`);
      }
    }
    ...
    assert.ok(down.removed.includes('class.null.level-3.ability-7'));
    ...
    const targetId = await director.mutation<string>('characters:create', {
      commandId: cid(),
      targetLevel: 1,
      authored: { name: `Null 3 target ${runId}`, appearance: '', biography: '', notes: '' },
    ...
    try {
          ...
          if (remainder) {
            ...
            if (compiled.has(name)) {
              ...
              const printed = (
                ledger.abilities as Record<
                  string,
                  { forcedMovementByTier?: (string | null)[] | null }
                >
              )[name]!.forcedMovementByTier?.[outcome.tier - 1];
              if (printed) {
                const [, vertical, kind, distance] = /^(vertical )?(slide|pull|push) (\d+)$/.exec(
                  printed,
                )!;
                const move = read.compiled.effects.find(o => o.effect.kind === 'push')!.effect;
                assert.equal(move.movement, kind, name);
                assert.equal(move.vertical === true, vertical !== undefined, name);
                assert.equal(move.printed, Number(distance), name);
              }
              if (name === 'Entropic Field') {
                // The target (v103-1) has Agility 2, never below the Null's 0/1/2 potencies.
                const slowed = read.compiled.effects.find(
                  o => o.effect.kind === 'condition',
                )!.effect;
                assert.equal(slowed.condition, 'slowed', name);
      ...
      assert.equal(used.size, 16, 'six level-2 and four level-3 abilities plus six embedded uses');
    } finally {
