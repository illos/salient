...
const revision = (path: string) => execFileSync('git', ['-C', path, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
...
function property(node: ts.ObjectLiteralExpression, name: string): ts.Expression | undefined {
  return node.properties.find((entry): entry is ts.PropertyAssignment =>
    ts.isPropertyAssignment(entry) && entry.name.getText() === name)?.initializer;
}
...
function array(node: ts.Expression | undefined): readonly ts.Expression[] {
  if (!node || !ts.isArrayLiteralExpression(node)) throw new Error('Expected a literal array; inspect upstream format change.');
  return node.elements;
}
function object(node: ts.Expression): ts.ObjectLiteralExpression {
  if (!ts.isObjectLiteralExpression(node)) throw new Error('Expected a literal object; inspect upstream format change.');
  return node;
}

async function inspect(path: string, kind: 'HeroClass' | 'SubClass') {
  const source = ts.createSourceFile(path, await readFile(join(forge, path), 'utf8'), ts.ScriptTarget.Latest, true);
  const definitions = source.statements.filter(ts.isVariableStatement)
    .flatMap(statement => [...statement.declarationList.declarations])
    .filter(declaration => declaration.type?.getText(source) === kind);
  if (definitions.length !== 1 || !definitions[0].initializer) throw new Error(`Expected one ${kind} in ${path}`);
  ...
  return {
    ...
    ...(kind === 'HeroClass' ? {
      ...
      subclassExports: array(property(definition, 'subclasses')).map(entry => entry.getText(source)),
    } : {}),
...
const classes: (DefinitionInventory & { compendiumId: string; subclasses: DefinitionInventory[] })[] = [];
for (const slug of (await readdir(join(forge, 'src/data/classes'))).sort()) {
  ...
  const data = JSON.parse(await readFile(join(compendium, `en/unified/json/class/${slug}.json`), 'utf8'));
  if (data.name !== main.name || typeof data.scc !== 'string') throw new Error(`Class mapping needs review: ${slug}`);
  const subclasses = [];
  for (const file of (await readdir(join(forge, directory))).sort()) {
    if (file.endsWith('.ts') && file !== `${slug}.ts`) subclasses.push(await inspect(`${directory}/${file}`, 'SubClass'));
  }
...
console.log(JSON.stringify({
  scope: 'Static class and subclass inventory. Factory calls are not resolved choices; defaults, referenced domains/kits, conditions, and option-to-SCC mappings are not evaluated.',
  forgeRevision: revision(forge), compendiumRevision: revision(compendium),
  compendiumClassesWithoutForgeDefinition: compendiumClasses.filter(slug => !classes.some(entry => entry.path === `src/data/classes/${slug}/${slug}.ts`)),
  classes,
}, null, 2));
