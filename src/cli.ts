...
import { abandonRequest, createRun, currentState, navigate, readRun, submitCommand, withRunFile, writeRun } from './history.ts';
...
export async function runCli(args: string[]): Promise<unknown> {
  ...
  const path = resolve(suppliedPath && suppliedPath !== '--force' ? suppliedPath : `.playtest/demo-${randomUUID()}.json`);
  if (operation === 'demo') return withRunFile(path, async () => {
    await ensureNew(path, args.includes('--force'));
    const run = createRun(await loadScenario());
    await writeRun(path, run);
    const results: HistoryEntry[] = [];
    results.push(await act(path, run, { kind: 'manual', id: 'start-turn', reason: 'Table supplied the starting turn Ferocity roll of 1; start-of-turn automation is outside this experiment.', changes: [{ kind: 'resource', entityId: 'fury', resource: 'ferocity', value: 1 }] }));
    results.push(await act(path, run, { kind: 'use-ability', id: 'goblin-strike', actorId: 'warrior', abilityId: 'warrior:spear-charge', targetIds: ['fury'], roll: [5, 5], facts: { targetsConfirmed: true, edges: 0, banes: 0, distances: { fury: 1 } } }));
    results.push(await act(path, run, { kind: 'use-ability', id: 'fury-slam', actorId: 'fury', abilityId: 'fury:brutal-slam', targetIds: ['warrior'], roll: [5, 5], facts: { targetsConfirmed: true, edges: 0, banes: 0, furyFerocityPeak: 2, distances: { warrior: 1 } } }));
    const pending = currentState(run).pending;
    if (pending.length) results.push(await act(path, run, { kind: 'manual', id: 'table-movement', reason: 'For this demonstration the table confirms it enacted the requested movement, with no collision or additional triggered effects.', changes: [], completePendingIds: pending.map(p => p.id) }));
    const beforeReopen = currentState(run);
    const reopened = await readRun(path);
    navigate(reopened, -1); const previous = currentState(reopened);
    navigate(reopened, 1); await writeRun(path, reopened);
    return { artifact: path, notes: run.scenario.notes, abilities: Object.fromEntries(['warrior:spear-charge', 'fury:brutal-slam'].map(id => [id, run.scenario.abilities[id]])), results, historyEvidence: { previous, beforeReopen, restored: currentState(reopened), explanation: 'Reopening and navigation read recorded snapshots. They do not invoke the evaluator or parser.' } };
  });
  if (operation === 'init') return withRunFile(path, async () => {
    await ensureNew(path, rest.includes('--force'));
    const run = createRun(await loadScenario({ includeSquad: rest.includes('--squad') }));
    await writeRun(path, run); return { artifact: path, scenario: run.scenario };
  });
  ...
  if (['state', 'log', 'abilities', 'ability', 'parse'].includes(operation)) {
    ...
    if (operation === 'state') return { cursor: run.cursor, entries: run.entries.length, state: currentState(run), unfinished: run.entries.filter(e => e.status === 'requested'), notes: run.scenario.notes };
    if (operation === 'log') return run;
    if (operation === 'abilities') return Object.values(run.scenario.abilities).map(a => ({ id: a.id, name: a.name, usage: a.usage, cost: a.cost, text: a.text, support: 'Use the explicit parse command to inspect current parser support; recorded outcomes remain in the log.' }));
    const ability = run.scenario.abilities[rest[0] ?? ''];
    if (!ability) throw new Error('Unknown ability ID');
    if (operation === 'parse') return { warning: 'Current parser analysis, not a replay of recorded actions or a guarantee of complete rules automation.', parsed: parseAbility(ability), notes: run.scenario.notes };
    return { ability, uses: run.entries.filter(e => e.command.kind === 'use-ability' && e.command.abilityId === ability.id) };
  }
  return withRunFile(path, async () => {
    ...
    if (operation === 'submit') {
      ...
      return { ability: command.kind === 'use-ability' ? run.scenario.abilities[command.abilityId] : undefined, entry, state: currentState(run) };
    }
    if (operation === 'back' || operation === 'forward') {
      ...
      await writeRun(path, run); return { cursor: run.cursor, entries: run.entries.length, state };
    }
    if (operation === 'abandon') {
      abandonRequest(run, rest[0] ?? '', rest.slice(1).join(' '));
      await writeRun(path, run); return run.entries.at(-1);
    }
