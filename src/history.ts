...
function validateState(state: unknown): asserts state is GameState {
  ...
  check(
    object(state.entities) && object(state.squads) && Array.isArray(state.pending),
    'Invalid state collections',
  );
  check(
    Number.isInteger(state.round) && state.round >= 1 && Number.isFinite(state.malice),
    'Invalid round or Malice',
  );
  for (const [id, e] of Object.entries(state.entities)) {
    check(object(e) && e.id === id && typeof e.name === 'string', 'Invalid entity identity');
    check(
      ['hero', 'monster'].includes(e.kind) && ['heroes', 'foes'].includes(e.side),
      'Invalid entity kind',
    );
    for (const key of ['stamina', 'maxStamina', 'temporaryStamina', 'size', 'stability', 'level'])
      check(Number.isFinite(e[key]), `Invalid entity ${key}`);
    check(
      object(e.characteristics) &&
        ['M', 'A', 'R', 'I', 'P'].every(k => Number.isFinite(e.characteristics[k])),
      'Invalid characteristics',
    );
    check(
      object(e.resources) && Object.values(e.resources).every(Number.isFinite),
      'Invalid resources',
    );
    check(
      Array.isArray(e.abilities) &&
        e.abilities.every((a: unknown) => typeof a === 'string') &&
        Array.isArray(e.traits) &&
        Array.isArray(e.conditions),
      'Invalid entity lists',
    );
    check(
      Array.isArray(e.meleeDamageBonus) &&
        e.meleeDamageBonus.length === 3 &&
        e.meleeDamageBonus.every(Number.isFinite),
      'Invalid damage bonuses',
    );
  }
  for (const [id, s] of Object.entries(state.squads)) {
    check(
      object(s) &&
        ...
        s.memberIds.every((m: string) => state.entities[m]?.squadId === id),
      'Invalid squad members',
    );
    check(
      ['stamina', 'maxStamina', 'memberStamina'].every(k => Number.isFinite(s[k])),
      'Invalid squad values',
    );
  }
  check(
    state.pending.every(
      (p: any) =>
        ...
        ['movement', 'manual'].includes(p.kind),
    ),
    'Invalid pending effects',
  );
  check(
    new Set(state.pending.map((p: any) => p.id)).size === state.pending.length,
    'Duplicate pending effect IDs',
  );
}
function validateCommand(command: unknown): asserts command is Command {
  check(
    object(command) && typeof command.id === 'string' && command.id.trim().length > 0,
    'Command requires a nonempty id',
  );
  check(command.kind === 'manual' || command.kind === 'use-ability', 'Unknown command kind');
  if (command.kind === 'manual')
    check(
      typeof command.reason === 'string' && command.reason.trim() && Array.isArray(command.changes),
      'Invalid manual command',
    );
  else
    check(
      typeof command.actorId === 'string' &&
        typeof command.abilityId === 'string' &&
        Array.isArray(command.targetIds) &&
        Array.isArray(command.roll) &&
        object(command.facts),
      'Invalid ability command',
    );
}
export function validateRun(value: unknown): asserts value is Run {
  ...
  check(
    object(value.scenario) &&
      ...
      typeof value.scenario.sourceRevision === 'string' &&
      Array.isArray(value.scenario.notes) &&
      object(value.scenario.abilities),
    'Invalid saved scenario',
  );
  validateState(value.scenario.state);
  for (const [id, a] of Object.entries(value.scenario.abilities))
    check(
      object(a) &&
        a.id === id &&
        ...
        object(a.source) &&
        typeof a.source.path === 'string',
      'Invalid saved ability',
    );
  check(
  ...
  for (const [index, entry] of value.entries.entries()) {
    check(
      object(entry) &&
        ['requested', 'completed', 'failed'].includes(entry.status) &&
        typeof entry.requestedAt === 'string',
      'Invalid history entry',
    );
    validateCommand(entry.command);
    check(!ids.has(entry.command.id), 'Duplicate command ID in saved history');
    ids.add(entry.command.id);
    validateState(entry.before);
    validateState(entry.after);
    check(isDeepStrictEqual(entry.before, previous), 'Broken history state continuity');
    if (entry.status === 'completed') {
      check(
        object(entry.output) &&
          ['resolved', 'needs-input', 'manual-required', 'rejected'].includes(
            entry.output.status,
          ) &&
          Array.isArray(entry.output.effects) &&
          Array.isArray(entry.output.messages) &&
          Array.isArray(entry.output.unresolved),
        'Invalid modifier output',
      );
      check(
        isDeepStrictEqual(entry.output.state, entry.after),
        'Output and recorded state disagree',
      );
      if (entry.output.status === 'rejected')
        check(isDeepStrictEqual(entry.before, entry.after), 'Rejected command changed state');
    } else {
      check(
        entry.output === undefined && isDeepStrictEqual(entry.before, entry.after),
        'Unfinished/failed entry changed state',
      );
      if (entry.status === 'requested')
        check(index === value.entries.length - 1, 'Unfinished request must be last');
      if (entry.status === 'failed')
        check(typeof entry.error === 'string', 'Missing failure reason');
    }
...
export function navigate(run: Run, delta: number): GameState {
  check(
    Number.isInteger(delta) && run.cursor + delta >= 0 && run.cursor + delta <= run.entries.length,
...
export async function submitCommand(
  ...
  command: Command,
  evaluator: Evaluator,
  options: { persist?: (run: Run) => Promise<void> } = {},
  ...
  check(
    run.cursor === run.entries.length,
    'History is in the past; move forward before submitting a new action',
  );
  check(
    !run.entries.some(e => e.status === 'requested'),
    'An unfinished request exists; explicitly abandon it before continuing',
  );
  ...
  try {
    ...
    Object.assign(candidate.entries.at(-1)!, {
      status: 'completed',
      output: clone(output),
      after: clone(output.state),
    });
...
export function abandonRequest(run: Run, id: string, reason: string): void {
  ...
  check(
    entry?.status === 'requested' && entry.command.id === id && reason.trim(),
    'Expected last unfinished request ID and a reason',
  );
  entry.status = 'failed';
  entry.error = `Explicitly abandoned: ${reason}`;
}
...
export async function withRunFile<T>(path: string, fn: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'EEXIST')
      throw new Error(
        `Run is locked: ${lockPath}. If its process crashed, remove this lock only after confirming no writer is running.`,
      );
    throw error;
  }
  try {
