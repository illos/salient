...
function expression(text: string): Expression | undefined {
  ...
  if (match && Number.isSafeInteger(Number(match[1])))
    return { constant: Number(match[1]), characteristic: match[2] as Characteristic };
  return undefined;
}
function effect(text: string): EffectDefinition | undefined {
  const value = plain(text);
  const damage =
    /^(.*?) (?:(acid|cold|corruption|fire|holy|lightning|poison|psychic|sonic) )?damage$/.exec(
      value,
    );
  if (damage) {
  ...
  if (push && Number.isSafeInteger(Number(push[1])))
    return { kind: 'push', distance: Number(push[1]) };
  const condition =
    /^([MARIP]) < (-?\d+),? (bleeding|dazed|frightened|grabbed|prone|restrained|slowed|taunted|weakened) \((save ends|EoT)\)$/.exec(
      value,
    );
  if (condition && Number.isSafeInteger(Number(condition[2])))
    return {
      kind: 'condition',
      characteristic: condition[1] as Characteristic,
      threshold: Number(condition[2]),
      condition: condition[3],
      duration: condition[4],
    };
  return undefined;
}
...
export function parseAbility(source: AbilitySource): ParsedAbility {
  ...
  if (frontmatter) {
    ...
    if (plain(declaredCost ?? '') !== plain(source.cost ?? ''))
      result.diagnostics.push('Structured cost differs from full text frontmatter.');
  }
  ...
  for (const original of body.split(/\r?\n/)) {
    ...
    if (!started && (title === source.name || title.startsWith(`${source.name} (`))) {
      const expected = source.cost
        ? `${source.name} (${plain(source.cost)})`
        : `${source.name} (Signature Ability)`;
      if (title !== expected && !(title === source.name && !source.cost))
        result.diagnostics.push(
          'Ability title contains unsupported or inconsistent cost/mechanics.',
        );
      started = true;
      continue;
    }
    ...
    if (line.startsWith('|')) {
      ...
      if (
        cells.length === 2 &&
        cells[0] === expectedKeywords &&
        cells[1] === plain(source.usage) &&
        headerRows === 0
      ) {
        headerRows++;
        continue;
      ...
      if (
        cells.length === 2 &&
        cells[0] === `📏 ${plain(source.distance)}` &&
        cells[1] === `🎯 ${plain(source.target)}` &&
        headerRows === 1
      ) {
        headerRows++;
        continue;
    ...
    if (roll) {
      if (rollText !== undefined)
        result.diagnostics.push('Multiple power rolls require manual resolution.');
  ...
  if (!rollText || tierTexts.some(tier => tier === undefined))
    result.diagnostics.push('Requires one power roll and all three outcome tiers.');
  if (source.roll && plain(source.roll) !== rollText)
    result.diagnostics.push('Structured roll differs from full text.');
  if (source.tiers && source.tiers.some((tier, index) => plain(tier) !== tierTexts[index]))
    result.diagnostics.push('Structured tiers differ from full text.');
  if (source.cost) {
    const cost = /^(\d+) (Ferocity|Malice)$/.exec(plain(source.cost));
    if (cost && Number.isSafeInteger(Number(cost[1])))
      result.cost = { resource: cost[2].toLowerCase(), amount: Number(cost[1]) };
