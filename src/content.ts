...
async function verifyRevision(root: string): Promise<void> {
  ...
  if (stdout.trim() !== SOURCE_REVISION)
    throw new Error(
      'Compendium revision differs from this experiment pin; review the content update before loading.',
    );
}
...
function ability(
  ...
  return {
    id,
    name: data.name,
    source: { path: `en/unified/md/${path}.md`, id: sourceId, revision: SOURCE_REVISION },
    text,
    usage: plain(data.usage ?? data.action_type ?? ''),
    distance: plain(data.distance ?? ''),
    target: plain(data.target ?? ''),
    keywords: (data.keywords ?? []).map(plain),
    ...(data.cost ? { cost: plain(data.cost) } : {}),
    ...(roll
      ? { roll: roll.roll, tiers: [roll.tier1, roll.tier2, roll.tier3] as [string, string, string] }
      : {}),
  };
}
async function record(path: string, root: string): Promise<{ data: RecordData; text: string }> {
  if (path.split('/').includes('..') || path.startsWith('/'))
    throw new Error('Content path must be relative to corpus category root.');
  await verifyRevision(root);
  const [json, text] = await Promise.all([
    readFile(join(root, `en/unified/json/${path}.json`), 'utf8'),
    readFile(join(root, `en/unified/md/${path}.md`), 'utf8'),
  ]);
...
export async function loadHeroAbility(
  ...
  if (kitCategory) {
    ...
    if (kit.data.signature_ability?.name !== source.name)
      throw new Error(`Ability is not the declared signature of kit ${kitCategory}`);
...
export async function loadMonster(
  ...
  for (const feature of data.features.filter(
    (entry: RecordData) => entry.feature_type === 'ability',
  )) {
    ...
    if (index < 0)
      throw new Error(`Cannot locate full ability text for ${feature.name} in ${path}`);
  ...
  const traits = data.features
    .filter((entry: RecordData) => entry.feature_type === 'trait')
    .map((entry: RecordData) => `source-trait:${slug(entry.name)}`);
  ...
  for (const [key, value] of Object.entries(data)) {
    if (!knownFields.has(key) && value !== undefined && value !== null)
      traits.push(`unsupported-field:${key}:${JSON.stringify(value)}`);
  }
  return {
    abilities,
    entity: {
      id: entityId,
      name: data.name,
      definitionId: data.metadata.scc,
      kind: 'monster',
      side: 'foes',
      level: data.level,
      stamina: Number(data.stamina),
      maxStamina: Number(data.stamina),
      temporaryStamina: 0,
      characteristics: {
        M: data.might,
        A: data.agility,
      ...
      ...(parseInt(data.size, 10) === 1 ? { sizeCategory: data.size } : {}),
      stability: data.stability,
      abilities: abilities.map(entry => entry.id),
      resources: {},
      conditions: [],
      traits,
      meleeDamageBonus: [0, 0, 0],
      freeStrikeDamage: data.free_strike,
    },
  };
}
...
export async function loadScenario(
  ...
  const sources = await Promise.all([
    ...selected.map(name =>
      loadHeroAbility(`feature/ability/fury/level-1/${name}`, `fury:${name}`, root),
    ),
    loadHeroAbility('feature/ability/mountain/pain-for-pain', 'mountain:pain-for-pain', root),
  ]);
  const warrior = await loadMonster(
    'monster/goblin/statblock/goblin-warrior',
    'warrior',
    'warrior',
    root,
  );
  ...
  const fury: Entity = {
    id: 'fury',
    name: 'Grug, Devil Berserker Fury',
    definitionId: 'fixture:level-1-devil-fury-mountain',
    kind: 'hero',
    side: 'heroes',
    level: 1,
    stamina: 30,
    ...
    sizeCategory: '1M',
    stability: 2,
    abilities: [...selected.map(name => `fury:${name}`), 'mountain:pain-for-pain'],
    resources: { ferocity: 0, victories: 0, recoveries: 10, surges: 0 },
    conditions: [],
    traits: [
      'fury:ferocity',
    ...
    fury: { windedTriggered: false },
  };
  const scenario: Scenario = {
    name: 'Level-one Devil Fury and Goblins',
    sourceRevision: SOURCE_REVISION,
    state: {
      entities: { fury, warrior: warrior.entity },
      squads: {},
      round: 1,
      malice: 0,
      pending: [],
  ...
  if (options.includeSquad) {
    const spine = await loadMonster(
      'monster/goblin/statblock/goblin-spinecleaver',
      'spine-1',
      'spine',
      root,
    );
    ...
    for (const id of memberIds)
      scenario.state.entities[id] = {
        ...structuredClone(spine.entity),
        id,
        name: `Goblin Spinecleaver ${id.slice(-1)}`,
        squadId: 'spine-squad',
      };
    scenario.state.squads['spine-squad'] = {
      id: 'spine-squad',
      memberIds,
      stamina: 20,
      maxStamina: 20,
    ...
    scenario.notes.push(
      'Four Spinecleavers share one 20-Stamina pool (5 each). Per-member Stamina is definition/display data; damage must use the squad pool. No captain.',
    );
  }
  return scenario;
}
