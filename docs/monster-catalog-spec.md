...
**Checkout status, 2026-09-14:** the app loads exactly one hardcoded stat block, the Goblin Warrior snapshot in
[shared/goblin-warrior.json](../shared/goblin-warrior.json), generated by
[scripts/build-foe-source.ts](../scripts/build-foe-source.ts) from the pinned Compendium revision
`fb83a789da8f0327a389c277a0c790b1648d5810` (tag `v4.20260908021459`) and checked by `pnpm foes:source`.
[convex/foes.ts](../convex/foes.ts) and [convex/foeTables.ts](../convex/foeTables.ts) provide Director-only
add/remove/detail, a `foes` table (immutable source snapshot, maximum Stamina, live Stamina) and a per-campaign
`foeSettings.addVisible` default. There is no catalog, search, importer, content package, saved encounter or
template, minion squad, or difficulty calculation. Per-foe show/hide and the Add-visibility default exist in code
