...
None at claim time. Any source ambiguity found during implementation is appended to
`docs/rules-questions-for-user.md` with its own id, and the rest of the unit continues.
...
1. **The whole configured browser suite**, not a five-spec selection: 48 passed, 2 failed, 3
   skipped in 19.6m, with the log retained. Both failures are account registration in
   `table-audit` and `v21-campaign`, specs this slice does not touch; their reruns are recorded in
   [the evidence README](evidence/V46/README.md#full-browser-suite).
