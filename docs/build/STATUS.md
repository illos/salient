...
| Id | Slice | Status | Owner |
| --- | --- | --- | --- |
| V50 | [Dwarf level one](V50-dwarf-level-one.md) | Preparation in progress — source inventory, three-build minimal-cover reference plan and derived expectations recorded on `slice/V50`. Rune timing resolved by a recorded interpretation (Q-CHAR-18 withdrawn): no rune at creation, and a level-one build with no rune is complete, so the runes are out of level-one build scope rather than a coverage gap. Five newly delivered options. Stand Tough confirmed not to raise Might; the counterpart builds cannot discriminate a wrong implementation, so a Dwarf Berserker Fury evaluator regression carries that check. No option enabled, no reference captured, no verification run. Implementation gated on the V46 pilot verdict | Opus implementer `88b6a7e6`, slice/V50 |
