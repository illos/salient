...
| Id | Slice | Status | Owner |
| --- | --- | --- | --- |
| V51 | [Timeout diagnostic](V51-timeout-diagnostic.md) | FIRST RUN VOID — [record](evidence/V51/README.md). I restored the slot before preserving artifacts, against an explicit instruction, and the raw data is destroyed and unrecoverable. Three further defects: loaded counts came from log history beyond the measurement window, the recorded source identity was wrong (actually b579450 dirty, not 7f2f28a clean), and the conclusions overreached from equal medians while tails degraded. All findings withdrawn; nothing here may be cited and no V46 gate may rest on it. Rerun needed with artifacts written to the persistent /artifacts mount and window-bounded aggregation | Opus `88b6a7e6`, slice/V51 |
