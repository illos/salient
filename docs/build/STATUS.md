...
| Id | Slice | Status | Owner |
| --- | --- | --- | --- |
| V51 | [Timeout diagnostic](V51-timeout-diagnostic.md) | MEASURED — [result](evidence/V51/README.md). The shared auth prefix is NOT the cause: it runs 43ms median under the load that produced the failure, while application queries on the same backend at the same moment run 500-1400ms. Simple host contention is also refuted, since the probes did not degrade while application queries did. The loaded condition reproduced the closeout failure, so this is a real refutation rather than an inconclusive run. Next suspect is the requireMember and tableContext layer, untested by either probe. Diagnostics remain deployed pending restore to b2c660a | Opus `88b6a7e6`, slice/V51 |
