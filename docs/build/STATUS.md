...
| Id | Slice | Status | Owner |
| --- | --- | --- | --- |
| V51 | [Timeout diagnostic](V51-timeout-diagnostic.md) | RERUN MEASURED — [result](evidence/V51/rerun.md), first run void and withdrawn ([record](evidence/V51/README.md)). Overlap verified: 192 application executions in the loaded window, 94 at or above 900ms, suite failed. Auth prefix median 42ms under load, unchanged from idle, against application medians near 1000ms, so it cannot account for their cost — but its tail does degrade (p90 54 to 204, max 77 to 490), so "refuted" is withdrawn. Bundle on CT114 at /srv/dev/salient/characters/artifacts/v51-rerun.tar.gz, sha256 b8aa2f8e. Not tested: requireMember and tableContext | Opus `88b6a7e6`, slice/V51 |
