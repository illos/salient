...
| V29 | [Desktop layout feedback follow-ups](V29-desktop-feedback.md) | V21 | Built on `slice/V29`, not merged — 2026-09-16 user desktop review (history controls into the settings pop-up; command errors as dismissible toasts); UI/polish track, worktree `ui`, isolated backend `anonymous:anonymous-agent`; lint, typecheck, 97 engine and 321 app tests, links, vendor, content and build pass; browser suite passes except the pre-existing stale content count in `table-audit.spec.ts:87`; the user's playable app on `5180` still runs `main` |
