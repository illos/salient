...
| Id | Slice | Status | Owner |
| --- | --- | --- | --- |
| V48 | [Elementalist level one](V48-elementalist-level-one.md) | Preparation reviewed and repaired — independent review returned four blocking findings, all fixed: wrong Celerity expectation, an acceptance check mandating two immunity entries against the pinned highest-wins rule, an acceptance check demanding skill coverage the matrix cannot deliver, and `amountOf` silently dropping `6 * echelon` so Permanence would have evaluated to nothing. Q-CHAR-16 and Q-CHAR-17 withdrawn as user questions and recorded as labelled interpretations in the owning documents: one ward damage type fixed at ward selection, and two distance increases combining to +4. Unrelated pre-existing defect recorded for V46: hero `damageWeaknesses` never reach `damageTargetFacts`, so they are inert in damage resolution. No option enabled, no capture, no verification run | Opus implementer `88b6a7e6`, slice/V48 |
