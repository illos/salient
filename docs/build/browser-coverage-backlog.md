...
| 2026-09-21 | V96 | Wizard rail: "Character Builder" heading with the Making a Hero rulebook link, steps 1–8 Ancestry…Determine Details, no Think or Make Connections row, current step's number badge in the accent, no pip | `v21-wizard.spec.ts` (obsolete: asserts `1. Think`, `10. Make Connections`, `Step 1 of 10`), `rule-popup.spec.ts` (obsolete: `Read 1. Think in the rules`) | spot | none needed: presentation only, content and headless routes unchanged (V96 work log) | |
