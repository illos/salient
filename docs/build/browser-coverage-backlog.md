...
| 2026-09-21 | V96 | Wizard rail: "Character Builder" heading with the Making a Hero rulebook link, steps 1–7 Ancestry…Determine Details, no Think, Add Free Strikes or Make Connections row, current step's number badge in the accent, no pip | `v21-wizard.spec.ts` (obsolete: asserts `1. Think`, `10. Make Connections`, `Step 1 of 10`), `rule-popup.spec.ts` (obsolete: `Read 1. Think in the rules`) | spot | none needed: presentation only, content and headless routes unchanged (V96 work log) | |
...
| 2026-09-21 | V96 | Ancestry step: each purchased-trait row shows its rules text wrapped under the name beside the point cost; rows stay aligned when the text runs to two lines; unsupported rows keep their muted reason | `wizard.spec.ts` | spot | catalog resolution checked for all 161 point-budget options (V96 work log) | |
