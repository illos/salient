...
future administrative sign-in, and the Convex Auth/Better Auth investigation. Convex is the data system; the
final authentication library remains open.
...
The [first table walkthrough](table-spec.md) establishes campaign/session/table hierarchy, session selection,
pause, role-dependent panes, free play, and the encounter loop. Detailed action economy and reaction handling
are explicitly deferred to a separate workstream. Continue remaining session/table product questions
independently of that work and the final screen layout; the confirmed high-level encounter loop remains in
place.
