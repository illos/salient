...
def render():
    ...
    classes = []
    for slug in [*CORE, "beastheart", "summoner"]:
        ...
        classes.append({"slug": slug, "sourceScope": "core" if slug in CORE else "supplemental (Q-CHAR-14)", **ref("class/" + slug),
    ...
    output = {"purpose": "Source inventory only. Counts, metadata and advancement cells are not evaluated decision definitions, reviewed effects or implemented support.",
              ...
              "scope": "Heroes book categories listed in categoryCounts. Core scope comes from the book-specific Git tree, not all unified records. Class advancement also inventories Beastheart and Summoner under Q-CHAR-14, explicitly supplemental.",
    ...
    lines = ["# V1 wizard source matrix", "", "Generated by `python3 docs/research/build-v1-wizard-index.py`; verify with `--check`.", "",
             ...
             "Core category membership is selected from the pinned Heroes book tree. Class advancement additionally includes Beastheart and Summoner under Q-CHAR-14; these remain supplemental, and inventory is not implementation. Every row retains its full SCC and source path in [the JSON index](v1-wizard-source-index.json).", "",
             "## Coverage denominators", "", "| Source category | Records |", "| --- | ---: |"]
    lines += [f"| {k} | {v} |" for k, v in counts.items()]
    lines += ["", "The 25 kits are 21 ordinary kits and four Stormwight kits. Skill records include skill-group entries; feature records include abilities, ancestry traits and parent sections. These are source-record counts, not player-choice counts. The 110 class/level rows (90 core plus 20 explicitly scoped supplemental rows) below require individual acceptance evidence before being called supported.", "",
    ...
    for c in classes:
        lines += [f'### {c["name"]}', "", f'Source: {link(c)} ({c["sourceScope"]}). Forge branch reference: ' + (", ".join(b["name"] for b in c["forgeBranchReferences"]) or "no SubClass objects; inspect domain choices") + ".", "",
