...
| 4 — settled | Must eligible level-ups be completed before the Director finishes respite, or may a hero finish those choices later? | Confirmed 2026-09-24: completion grants pending level-ups (one per threshold crossed); the owner takes each later as a separate sheet action. The respite and the next session never wait for them. |
| 5 — settled | How should the Director record an interruption and move back into play? | Confirmed 2026-09-24: three endings. Cancel reverts every respite change to the pre-respite state; Interrupt keeps what already happened but grants no completion benefits and returns to ordinary play (not a combat void); Complete applies the benefits. |
| 6 — settled | Should optional ability swapping and alternative advancement modes have dedicated V1 controls? | Confirmed 2026-09-24: a manual Director grant creates a pending level-up (covers Director Says So); adjusted XP pace is the campaign XP-per-level setting; milestone advancement is not in V1. Respite ability changes (optional rule): not in V1; players use the full edit with Director approval. |
| 7 — settled | How should the Director finish when optional respite choices remain unused? | Confirmed 2026-09-24: no ready-state gate (players confirm verbally); unused options lapse on Complete but stay clearly visible per hero, styled like the wizard's unspent-points notice. |
| 8 — settled | Can a completed respite be reversed, and how far can that reversal reach? | Confirmed 2026-09-24: no. Complete is final; corrections use the existing Director adjustments. Cancel (revert) applies only to an open respite. |
