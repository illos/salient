...
Nothing here is implemented yet. The font is not in `web/` and no component consumes it; this document
is the specification a future slice builds against. Requirements are labelled **confirmed** (observed in
...
The reason is licensing rather than engineering. CC BY-SA 4.0 §2(a)(4) permits format shifting, so a
WOFF2 build would be lawful, but any modified font is an Adapted Work carrying ShareAlike obligations of
its own. Shipping the file verbatim means the project redistributes and never adapts. The saving from
subsetting is roughly 9 KB against a 21.6 KB original and does not justify the added licence surface.
...
Corpus marker counts below were measured over the pinned submodule with `grep`.
...
The Compendium writes all 3,034 occurrences in the uniform form `X < Y`, where `X` is one of `M A I R P`
and `Y` is a positive integer or one of `WEAK`, `AVERAGE`, `STRONG`. That regularity makes detection
...
This font is a sharper hazard than a typical icon font, and the reason is structural: **the glyphs are
mapped onto ordinary ASCII and Latin-1 codepoints, not a Private Use Area.** The melee icon is the
letter `t`. A potency pill is the literal text `a<1`. Left alone, a screen reader announces "t" for a
sword, and copying an ability yields `t` where "Melee" belongs.

The contract follows from that. All of it is **chosen**, and all of it is mandatory.

### 6.1 A glyph never carries meaning by itself

Every glyph run is wrapped in a span with `aria-hidden="true"`. Meaning is carried by real text: either
adjacent visible text, or a visually hidden `sr-only` span using the accessible name from section 3.

### 6.2 Do not use `role="img"` with `aria-label`

That pattern would give correct speech, but the glyph character stays in the accessible text and in the
document's text content, so selection and copy still produce `t`. The `aria-hidden` plus `sr-only`
pairing fixes speech and the clipboard together, because the hidden text is what gets copied.

### 6.3 Suppress the label when adjacent text already says it

Where a glyph precedes text that states the same word inside the same accessible grouping, the glyph
takes no `sr-only` label and is purely decorative. The concrete case: an ability card whose keyword row
already reads "Melee, Strike, Weapon" does not also announce "Melee" for the leading icon. Where the
glyph is the only thing conveying the category — the distance and target lines, whose text reads
`Ranged 15` and `One creature` without naming which is which — the label is required.

### 6.4 Selection and copy

Glyph spans set `user-select: none`, so a selection spanning an ability yields the `sr-only` text and
ordinary prose, and never a stray `t` or `a<1`.

### 6.5 Font loading

`@font-face` uses `font-display: block`. A glyph that has not loaded must render as nothing rather than
briefly flashing the wrong letter. The font is bundled and fingerprinted by Vite, so a failed fetch is
not an expected condition, but the failure mode of showing `t` is bad enough to warrant the setting.

### 6.6 Colour and forced colours

Glyph tiles are filled shapes whose letters are counterforms, not painted white. They therefore inherit
`currentColor` and invert correctly between light and dark themes with no per-theme handling, verified
by rendering both. No glyph may be given a hard-coded colour; this is also required by the design
system's no-raw-hex acceptance check.

### 6.7 Rules reader sanitisation

`web/rules/article.tsx` renders article HTML through `rehypeSanitize`. Admitting glyphs requires
extending the schema to permit exactly `<span class="ds-glyph" aria-hidden="true">` and
`<span class="sr-only">`. The allowance stays that narrow: the project generates this markup, so nothing
broader is needed.

### 6.8 Required tests

- No element's accessible name contains a bare glyph character. Assert over ability cards, stat blocks
  and rules articles.
- An ability card's accessible name spells out distance, target, action category and every potency.
- A characteristic row announces `Might +3`, not `M` or `ight`.
- Ornament runs expose no accessible name at all.

## 7. Component contract

**Chosen, proposed for the implementing slice.**

- `web/components/glyph.tsx` exports a `GlyphName` union. Every name maps to its codepoint and its
  default accessible name from section 3. No component writes a raw glyph character; a raw character
  outside this module is a defect.
- `<Glyph name="melee" />` is **labelled by default** and emits the `aria-hidden` run plus the `sr-only`
  name. `<Glyph name="melee" decorative />` suppresses the label for the section 6.3 case. Defaulting to
  labelled means the unsafe state has to be asked for.
- Ornaments are a separate export that cannot take a label.
- Potency renders through its own component, which owns the whole run, emits the proportional form for
  every element including digits, and produces one `sr-only` name for the pill.
- Everything else emits the em-square form. Callers never choose between the families; section 4 is
  enforced by which component is used.

## 8. Where glyphs appear in Salient

**Chosen.** Applies wherever the underlying data already carries the marker.

| Surface | Glyphs |
| --- | --- |
| Ability cards (`web/character-sheet/ability-card.tsx`) | Category icon, distance, target, tier pills, potency |
| Foe stat blocks | The same, plus trait and leader-or-solo markers and characteristic rows |
| Rules reader articles (`web/rules/`) | All of the above as printed, plus ornaments |
| Table log and Malice display | Leader-or-solo marker only |

The books apply the full treatment in their densest layouts — Monsters p33 carries about fourteen
potency pills on one page and stays readable — so density is not a reason to withhold potency glyphs
from ability cards.

One pipeline change is required before any of this can render. `scripts/ingest-rules.ts:183-185`
currently **discards** exactly these markers, rewriting 📏 and 🎯 to text labels and deleting the rest;
`web/rules/reference.ts:62` strips 📏 and 🎯 again for card labels. Both must preserve the markers as
semantic tokens instead. `public/rules-data/` is generated and ignored by Git, so no committed snapshot
churns. `convex/lib/resolve.ts` reads raw Compendium text and is unaffected.
