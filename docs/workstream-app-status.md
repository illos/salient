...
Next action: consume the rules thread's first reviewed character/combat contract and implement the dependent journey, preserving the current identity, revision and audience checks. App changes are currently uncommitted after baseline `5efb7c7`; separate rules-thread document edits also remain in the shared checkout.
