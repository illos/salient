...
function SourceText({ sourceSnapshot }: { sourceSnapshot: string }) {
  ...
  return (
    <details>
      <summary>Read complete source: {source.name}</summary>
  ...
  return (
    <div className="panel stack">
      <div className="row">
        ...
        <span className="muted">{foe.visible ? 'Visible to players' : 'Hidden from players'}</span>
      </div>
      <div className="row">
        <button
  ...
  return (
    <div className="stack">
      ...
      {source ? (
        ...
        <p role="status">Loading available foe…</p>
      )}
      {addition.error && (
        <p className="error" role="alert">
          {addition.error}
