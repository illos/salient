...
export function errorMessage(error: unknown): string {
  ...
  return message.replace(/\[CONVEX[^\]]*\]\s*/g, '').replace(/\[Request ID:[^\]]*\]\s*/g, '').split('\n    at ')[0];
}
...
export function useCommand() {
  ...
  async function run(fn: (commandId: string) => Promise<unknown>, payloadKey: string): Promise<boolean> {
    if (inFlight.current) return false;
    inFlight.current = true; setPending(true); setError(null);
    const commandId = commands.current.forPayload(payloadKey);
    try { await fn(commandId); commands.current.acknowledged(payloadKey); return true; }
