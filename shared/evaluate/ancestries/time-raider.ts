...
export function applyTimeRaiderBaseline(
  ...
  const base = (amount?: number): Provenance => ({
    decisionId: 'ancestry.time-raider.base-statistics',
  ...
  const psychic: Provenance = {
    decisionId: 'ancestry.time-raider.signature-trait',
    source: ctx.sentence({
      path: 'en/unified/md/feature/trait/time-raider/psychic-scar.md',
      quote: 'You have psychic immunity equal to your level.',
    }),
    operation: 'set',
    amount: ctx.level,
  };
  (out.damageImmunities ??= []).push({
    damageType: 'psychic',
    value: { value: ctx.level, provenance: [psychic] },
  });
  if ((ctx.list('ancestry.time-raider.purchased-traits') ?? []).includes('Unstoppable Mind'))
    (out.conditionImmunities ??= []).push({
      ...
      provenance: {
        ...
        source: ctx.sentence({
          path: 'en/unified/md/feature/trait/time-raider/time-raider-traits.md',
