...
export function applyWodeElfBaseline(
  ...
  const base = (amount?: number): Provenance => ({
    decisionId: 'ancestry.wode-elf.base-statistics',
  ...
  const purchased = (name: string, slug: string, quote: string, amount: number): Provenance => ({
    decisionId: 'ancestry.wode-elf.purchased-traits',
    selection: name,
    source: ctx.sentence({ path: `en/unified/md/feature/trait/wode-elf/${slug}.md`, quote }),
    operation: 'set',
    amount,
  });
  ...
  out.speed = {
    value: (swift ? 6 : 5) + (out.kit?.speedBonus.value ?? 0),
    provenance: [
      swift ? purchased('Swift', 'swift', 'You have speed 6.', 6) : base(5),
  ...
  if (selected.includes('Otherworldly Grace'))
