...
export function applyTalentModifiers(ctx: DerivationContext, out: PartialBaseline) {
  if (ctx.single('class.choice') !== 'Talent') return;
  const source = (id: string, slug: string, quote: string, amount?: number): Provenance => ({
    decisionId: `class.talent.${id}`,
    source: ctx.sentence({ path: `en/unified/md/feature/talent/level-1/${slug}.md`, quote }),
    ...(amount === undefined ? {} : { operation: 'add', amount }),
  });
  const augmentation = ctx.single('class.talent.augmentation');
  const add = (
    ...
    if (out.staminaMaximum) {
      out.recoveryValue = {
        ...
        provenance: [
          ...
          {
            decisionId: 'class.talent.augmentation',
